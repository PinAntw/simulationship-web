...
const PairingPanel = ({ onPairsChange }) => {
  ...
  const addPair = () => {
    if (!a || !b || a === b) return;
    if (pairs.some(([x, y]) => (x === a && y === b) || (x === b && y === a))) return;
  ...
  const savePairsToServer = async () => {
    ...
    try {
      ...
      alert(UI_TEXT.MSG_PAIR_SENT); // [修改]
    } catch (e) {
      console.error(e);
      alert(UI_TEXT.MSG_PAIR_FAIL); // [修改]
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded p-3 space-y-3">
      <div className="font-bold text-sm mb-1">{UI_TEXT.MATCH_TITLE}</div>
      ...
      <div className="flex gap-2">
        ...
        >
          <option value="">{UI_TEXT.PH_SELECT_A}</option>
          {characters.map((c) => (
        ...
        >
          <option value="">{UI_TEXT.PH_SELECT_B}</option>
          {characters.map((c) => (
            <option key={c.id} value={c.name}>{c.name}</option>
          ))}
        </select>
      ...
      <div className="text-xs text-gray-300">
        {pairs.length === 0 && <span className="text-gray-500 ml-1">{UI_TEXT.LIST_EMPTY_SIMPLE}</span>}
      </div>
      ...
      >
        {saving ? UI_TEXT.BTN_SENDING : UI_TEXT.BTN_SEND_BACKEND}
      </button>
    </div>
  );
};
